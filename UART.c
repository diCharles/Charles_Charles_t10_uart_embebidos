...
void UART_init(uart_channel_t uart_channel, uint32_t system_clk , uart_baud_rate_t baud_rate)
{
	...
	case  UART_0 :
		...
		UART0->BDH |= (baud_rate>>8) & (UART_BDH_SBR_MASK) ;  /* Copiar los bits UART baud rate [12:8] a los bits SRB del registro UART0_BDH */
...
void UART_put_char (uart_channel_t uart_channel, uint8_t character)
{
	...
	{
	case UART_0:
		while(0 != (UART0->S1 & UART_S1_TDRE_MASK ));/* wait until flag is on zero*/
		UART0->D = character ;
		break;
	case UART_1:
		...
		break;
	case UART_2:
		while(0 != (UART2->S1 & UART_S1_TDRE_MASK ));/* wait until flag is on zero*/
		UART2->D = character ;
		break;
	case UART_3:
		while(0 != (UART3->S1 & UART_S1_TDRE_MASK ));/* wait until flag is on zero*/
		UART3->D = character ;
		break;
	case UART_4:
		while(0 != (UART4->S1 & UART_S1_TDRE_MASK ));/* wait until flag is on zero*/
		UART4->D = character ;
		break;
	case UART_5:
		while(0 != (UART5->S1 & UART_S1_TDRE_MASK ));/* wait until flag is on zero*/
		UART5->D = character ;
		break;
	default:
		break;
